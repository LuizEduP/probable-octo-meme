...
const insert = db.prepare(`
  ...
  VALUES (?, ?, ?, ?, '/uploads/sem-foto.svg', ?, ?, ?)
`);

const insertMany = db.transaction((items) => {
  for (const p of items) {
    insert.run(p.nome, p.descricao, p.preco, p.preco_original, p.categoria_id, Math.floor(Math.random() * 50) + 5, p.destaque);
  }
});
...
console.log('📦 Categorias: Eletrônicos, Moda, Casa, Esportes, Livros, Brinquedos, Beleza, Alimentos');
db.close();
